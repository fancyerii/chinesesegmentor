...
public class InstanceGenerator {
  ...
  public static class CounterMapper extends Mapper<Object, Text, IntWritable, Text> {
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
      ...
      try {
        foi = new FSTObjectInput(new FileInputStream(paths[0].toString()));
        try {
      ...
      try {
        template = (Template) string2Object(context.getConfiguration().get("template"));
      } catch (ClassNotFoundException e) {
        throw new IOException(e);
      }
    ...
    public void map(Object key, Text value, final Context context) throws IOException,
      ...
      Instance instance = SgdCrf.buildInstance(tks, tc, fd, labelDict, template, false, false);
