	...
	public static class TrainingMapper extends Mapper<Object, Text, Text, DoubleWritable> {
					...
					if (stat.isDir()) {
						...
						if (f.getName().startsWith("_")) {
							...
							try{
								...
								Writable key = (Writable) ReflectionUtils.newInstance(reader.getKeyClass(), context.getConfiguration());
								Writable value = (Writable) ReflectionUtils.newInstance(reader.getValueClass(), context.getConfiguration());
		...
		private void doSgd(final Context context) throws IOException, InterruptedException{
			...
			for(int i=0;i<weights.getAttributeWeights().length;i++){
				context.write(new Text(FEATURE_WEIGHT+i), new DoubleWritable(weights.getAttributeWeights()[i]));
			}
			for(int i=0;i<weights.getBosTransitionWeights().length;i++){
				context.write(new Text(BOS_WEIGHT+i), new DoubleWritable(weights.getBosTransitionWeights()[i]));
			}
			for(int i=0;i<weights.getEosTransitionWeights().length;i++){
				context.write(new Text(EOS_WEIGHT+i), new DoubleWritable(weights.getEosTransitionWeights()[i]));
			}
			
			for(int i=0;i<weights.getTransitionWeights().length;i++){
				context.write(new Text(TRANSITION_WEIGHT+i), new DoubleWritable(weights.getTransitionWeights()[i]));
			}
		}
