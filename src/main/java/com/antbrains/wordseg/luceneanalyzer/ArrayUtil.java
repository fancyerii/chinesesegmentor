...
public final class ArrayUtil {
   ...
   * /java6/modules/luni/src/main/java/java/lang/Integer.java
   */
   ...
   *          Bytes used by each element of the array. See constants in {@link RamUsageEstimator}.
   *
   * @lucene.internal
   */
   ...
   * Class#getComponentType() and Array.newInstance() public static <T> T[] grow(T[] array, int
   * minSize) { if (array.length < minSize) {
   * 
   * @SuppressWarnings("unchecked") final T[] newArray = (T[])
   * Array.newInstance(array.getClass().getComponentType(), oversize(minSize,
   * RamUsageEstimator.NUM_BYTES_OBJECT_REF)); System.arraycopy(array, 0, newArray, 0,
   * array.length); return newArray; } else return array; }
   * 
   * public static <T> T[] grow(T[] array) { return grow(array, 1 + array.length); }
   * 
   * public static <T> T[] shrink(T[] array, int targetSize) { final int newSize =
   * getShrinkSize(array.length, targetSize, RamUsageEstimator.NUM_BYTES_OBJECT_REF); if (newSize !=
   * array.length) {
   * 
   * @SuppressWarnings("unchecked") final T[] newArray = (T[])
   * Array.newInstance(array.getClass().getComponentType(), newSize); System.arraycopy(array, 0,
   * newArray, 0, newSize); return newArray; } else return array; }
   */
  ...
  /**
   * Sorts the given array using the {@link Comparator}. This method uses the quick sort algorithm,
  ...
  /**
   * Sorts the given array slice in natural order. This method uses the quick sort algorithm, but
  ...
  /**
   * Sorts the given array in natural order. This method uses the quick sort algorithm, but falls
  ...
  /**
   * Sorts the given array using the {@link Comparator}. This method uses the merge sort algorithm,
   * but falls back to insertion sort for small arrays.
   */
  ...
  /**
   * Sorts the given array slice in natural order. This method uses the merge sort algorithm, but
   * falls back to insertion sort for small arrays.
   * 
   * @param fromIndex
   *          start index (inclusive)
  ...
  /**
   * Sorts the given array in natural order. This method uses the merge sort algorithm, but falls
   * back to insertion sort for small arrays.
   */
  ...
  /**
   * Sorts the given array slice in natural order. This method uses the insertion sort algorithm. It
   * is only recommended to use this algorithm for partially sorted small arrays!
   * 
   * @param fromIndex
   *          start index (inclusive)
   * @param toIndex
   *          end index (exclusive)
   */
  public static <T extends Comparable<? super T>> void insertionSort(T[] a, int fromIndex,
      int toIndex) {
    if (toIndex - fromIndex <= 1)
      return;
  ...
  /**
   * Sorts the given array in natural order. This method uses the insertion sort algorithm. It is
   * only recommended to use this algorithm for partially sorted small arrays!
   */
