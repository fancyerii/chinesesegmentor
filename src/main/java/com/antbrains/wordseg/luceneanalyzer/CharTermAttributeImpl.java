 ...
 */
public class CharTermAttributeImpl extends AttributeImpl implements CharTermAttribute,
    TermToBytesRefAttribute, Cloneable {
  ...
  private char[] termBuffer = new char[ArrayUtil.oversize(MIN_BUFFER_SIZE, NUM_BYTES_CHAR)];
  ...
  public final char[] resizeBuffer(int newSize) {
    if (termBuffer.length < newSize) {
      ...
      final char[] newCharBuffer = new char[ArrayUtil.oversize(newSize, NUM_BYTES_CHAR)];
  ...
  private void growTermBuffer(int newSize) {
    if (termBuffer.length < newSize) {
      // Not big enough; create a new array with slight
      // over allocation:
      termBuffer = new char[ArrayUtil.oversize(newSize, NUM_BYTES_CHAR)];
    }
  }
  ...
  public final CharTermAttribute append(CharSequence csq, int start, int end) {
    ...
    if (len > 4) { // only use instanceof check series for longer CSQs, else simply iterate
      if (csq instanceof String) {
        ...
        System.arraycopy(cb.array(), cb.arrayOffset() + cb.position() + start, termBuffer,
            termLength, len);
   ...
   * the whole term with all attributes. This affects especially the
