...
public class AttributeSource {
  ...
  public static abstract class AttributeFactory {
    ...
    /**
     * This is the default factory that creates {@link AttributeImpl}s using the class name of the
     * supplied {@link Attribute} interface class by appending <code>Impl</code> to it.
     */
        ...
        synchronized (attClassImplMap) {
          ...
          if (clazz == null) {
            try {
              attClassImplMap.put(
                  attClass,
                  new WeakReference<Class<? extends AttributeImpl>>(clazz = Class.forName(
                      attClass.getName() + "Impl", true, attClass.getClassLoader()).asSubclass(
                      AttributeImpl.class)));
  ...
  /**
   * An AttributeSource using the supplied {@link AttributeFactory} for creating new
  ...
  static LinkedList<WeakReference<Class<? extends Attribute>>> getAttributeInterfaces(
    ...
    synchronized (knownImplClasses) {
      ...
      if (foundInterfaces == null) {
        // we have a strong reference to the class instance holding all interfaces in the list
        // (parameter "att"),
        ...
        do {
          for (Class<?> curInterface : actClazz.getInterfaces()) {
            if (curInterface != Attribute.class && Attribute.class.isAssignableFrom(curInterface)) {
              foundInterfaces.add(new WeakReference<Class<? extends Attribute>>(curInterface
   ...
   * <p>
   * <font color="red"><b>Please note:</b> It is not guaranteed, that <code>att</code> is added to
   * the <code>AttributeSource</code>, because the provided attributes may already exist. You should
   * always retrieve the wanted attributes using {@link #getAttribute} after adding with this method
   * and cast to your class. The recommended way to use custom implementations is using an
   * {@link AttributeFactory}. </font>
   * </p>
   */
  ...
  /**
   * The caller must pass in a Class&lt;? extends Attribute&gt; value. Returns the instance of the
   * passed in Attribute contained in this AttributeSource
   * 
   * @throws IllegalArgumentException
   *           if this AttributeSource does not contain the Attribute. It is recommended to always
   *           use {@link #addAttribute} even in consumers of TokenStreams, because you cannot know
   *           if a specific TokenStream really uses a specific Attribute. {@link #addAttribute}
   *           will automatically make the attribute available. If you want to only use the
   *           attribute, if it is available (to optimize consuming), use {@link #hasAttribute}.
   */
  public final <A extends Attribute> A getAttribute(Class<A> attClass) {
    AttributeImpl attImpl = attributes.get(attClass);
    if (attImpl == null) {
      throw new IllegalArgumentException("This AttributeSource does not have the attribute '"
          + attClass.getName() + "'.");
    }
    return attClass.cast(attImpl);
  }
   ...
   * <p>
   * Note that this method does not affect attributes of the targetStream that are not contained in
   * this state. In other words, if for example the targetStream contains an OffsetAttribute, but
   * this state doesn't, then the value of the OffsetAttribute remains unchanged. It might be
   * desirable to reset its value to the default, in which case the caller should first call
   * {@link TokenStream#clearAttributes()} on the targetStream.
   */
   ...
   * {@link AttributeImpl#reflectWith} method.
   * </p>
   *
   ...
   * with exactly the same attributes (using {@link #AttributeSource(AttributeSource)}). You can
   * also use it as a (non-performant) replacement for {@link #captureState}, if you need to look
   ...
   * . The given instance has to provide all {@link Attribute}s this instance contains. The actual
   * attribute implementations must be identical in both {@code AttributeSource} instances; ideally
   * both AttributeSource instances should use the same {@link AttributeFactory}. You can use this
   * method as a replacement for {@link #restoreState}, if you use {@link #cloneAttributes} instead
   * of {@link #captureState}.
   */
