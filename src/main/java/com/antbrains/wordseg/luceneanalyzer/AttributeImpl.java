 ...
 * <p>
 * Attributes are used to add data in a dynamic, yet type-safe way to a source of usually streamed
 * objects, e. g. a {@link org.apache.lucene.analysis.TokenStream}.
 */
public abstract class AttributeImpl implements Cloneable, Attribute {
   ...
   *
   * <p>
   * The default implementation calls {@link AttributeReflector#reflect} for all non-static fields
   * from the implementing class, using the field name as key and the field value as value. The
   * Attribute class is also determined by reflection. Please note that the default implementation
   * can only handle single-Attribute implementations.
   *
   * <p>
   ...
   * {@link Attribute} interfaces and keys are passed to {@link AttributeReflector#reflect} in the
   * same order, but possibly different values. So don't automatically exclude e.g. {@code null}
   * properties!
   *
